...
console.log(
  `[PathCheck] Default DB Path used (relative to CWD): ${defaultDbPath}`,
);
console.log(
  `[PathCheck] Current working directory (via __dirname): ${__dirname}`,
);
console.log(`[PathCheck] Resolved absolute DB Path: ${resolvedDbPath}`);

if (fs.existsSync(resolvedDbPath)) {
  console.log(
    `[PathCheck] SUCCESS: Database file FOUND at resolved path: ${resolvedDbPath}`,
  );
} else {
  console.error(
    `[PathCheck] ERROR: Database file NOT FOUND at resolved path: ${resolvedDbPath}`,
  );
}
