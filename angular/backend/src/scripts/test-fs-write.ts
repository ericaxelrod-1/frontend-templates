...
try {
  if (!fs.existsSync(logsDir)) {
    console.log(
      `[FS Test] Logs directory does not exist, attempting to create: ${logsDir}`,
    );
  ...
  console.log(
    `[FS Test] Successfully wrote to ${testFilePath}. Checking content...`,
  );

  const writtenContent = fs.readFileSync(testFilePath, 'utf-8');
  if (writtenContent.includes('Filesystem test content')) {
    console.log(
      `[FS Test] Content verification successful! File content:\n${writtenContent}`,
    );
  } else {
    console.error(
      `[FS Test] Content verification FAILED. File exists but content is unexpected or empty. Read: ${writtenContent}`,
    );
  }
} catch (error) {
