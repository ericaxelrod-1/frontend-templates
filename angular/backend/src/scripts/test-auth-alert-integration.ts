...
async function testAuthAlertIntegration() {
  ...
  try {
    ...
    const loginFailureAlert = {
      ...
      message:
        'Failed login attempt for user admin@test.com from IP 192.168.1.103',
    ...
    const loginFailureAlerts = await securityAlertService.getSecurityAlerts(
      10,
      0,
      {
    ...
    const suspiciousAlerts = await securityAlertService.getSecurityAlerts(
      10,
      0,
      {
    ...
    for (const alert of recentAlerts.items.slice(0, 5)) {
      console.log(
        `- [${alert.severity.toUpperCase()}] ${alert.title} (${alert.createdAt})`,
      );
      if (alert.alertData) {
    ...
    const highSeverityAlerts = await securityAlertService.getSecurityAlerts(
      10,
      0,
      {
        severity: 'high',
    ...
    const mediumSeverityAlerts = await securityAlertService.getSecurityAlerts(
      10,
      0,
      {
        severity: 'medium',
