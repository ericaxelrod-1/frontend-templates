...
async function testLoginResponse() {
  ...
  try {
    ...
    logger.log(
      'Permissions array:',
      Array.isArray(loginResponse.user.permissions),
    );
    logger.log(
      'Permissions count:',
    ...
    if (
      ...
      if (typeof loginResponse.user.permissions[0] === 'string') {
        ...
        const loginMonitoringPerms = loginResponse.user.permissions.filter(
          (p) => p.includes('login-monitoring'),
        );
        ...
        logger.log(
          'Structure:',
          JSON.stringify(loginResponse.user.permissions[0], null, 2),
        );
      }
