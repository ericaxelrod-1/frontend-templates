...
import {
  DetectedPattern,
  PatternType,
} from '../modules/auth/services/pattern-detection.service';
...
async function testPatternAlertIntegration() {
  ...
  try {
    ...
    const patternAlertResult =
      await alertService.sendPatternAlert(bruteForcePattern);
    ...
    const credentialStuffingPattern: DetectedPattern = {
      ...
      userId: null,
      details:
        'Credential stuffing attack detected - rapid login attempts with different credentials',
      evidence: {
        ipAddress: '192.168.1.102',
        attemptCount: 15,
    ...
    const credentialStuffingResult = await alertService.sendPatternAlert(
      credentialStuffingPattern,
    );
    console.log(
      `Credential stuffing alert sent result: ${credentialStuffingResult}`,
    );

    // Test 4: Verify credential stuffing alert was stored
    console.log('\n🔍 Test 4: Checking for credential stuffing alerts...');
    const credentialAlerts = await securityAlertService.getSecurityAlerts(
      10,
      0,
      {
    ...
    console.log(
      `Found ${credentialAlerts.total} credential stuffing alerts in database`,
    );
    ...
    for (const alert of allAlerts.items) {
      alertTypeBreakdown[alert.alertType] =
        (alertTypeBreakdown[alert.alertType] || 0) + 1;
    }
