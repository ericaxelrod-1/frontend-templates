...
async function testEnhancedPatternDetection() {
  ...
  try {
    ...
    if (historicalPatterns.length > 0) {
      ...
      console.log(`   Sample: ${sample.type} - ${sample.severity} - ${sample.details}`);
    ...
    console.log(`✅ Found ${combinedPatterns.length} total patterns (real-time + historical)`);
    
    const realTimeCount = combinedPatterns.filter(p => !p.isHistorical).length;
    const historicalCount = combinedPatterns.filter(p => p.isHistorical).length;
    ...
    console.log(`✅ Real-time detection after test: ${newRealTimePatterns.length} patterns`);
    
    if (newRealTimePatterns.length > 0) {
      const bruteForcePattern = newRealTimePatterns.find(p => p.type === 'brute_force');
      if (bruteForcePattern) {
        console.log(`   🎯 Detected brute force pattern: ${bruteForcePattern.details}`);
    ...
    const distributedPattern = distributedPatterns.find(p => p.type === 'distributed_attack');
    if (distributedPattern) {
      console.log(`   🎯 Detected distributed attack: ${distributedPattern.details}`);
    }
