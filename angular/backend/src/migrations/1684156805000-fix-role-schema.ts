  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      if (table) {
        ...
        const hasSystemRole = table.columns.some(
          (column) => column.name === 'is_system_role',
        );
        if (!hasSystemRole) {
        ...
        const hasDefault = table.columns.some(
        ...
        const hasParentId = table.columns.some(
          (column) => column.name === 'parent_id',
        );
        if (!hasParentId) {
          await queryRunner.query(
            `ALTER TABLE roles ADD COLUMN parent_id INTEGER NULL`,
          );
          console.log('Added parent_id column to roles table');
        }

        // Add priority column if it doesn't exist
        const hasPriority = table.columns.some(
          (column) => column.name === 'priority',
        );
        if (!hasPriority) {
          await queryRunner.query(
            `ALTER TABLE roles ADD COLUMN priority INTEGER NULL`,
          );
