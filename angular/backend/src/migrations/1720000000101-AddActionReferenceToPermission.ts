...
export class AddActionReferenceToPermission1720000000101 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!hasActionIdColumn) {
      await queryRunner.query(`ALTER TABLE "permissions" ADD "action_id" integer`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasForeignKey = await queryRunner.hasColumn('permissions', 'action_id');
    if (hasForeignKey) {
      try {
        await queryRunner.query(`ALTER TABLE "permissions" DROP CONSTRAINT "FK_permissions_action_id"`);
      } catch (error) {
