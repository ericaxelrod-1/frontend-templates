...
export class UpdatePermissionEntity1720000000100 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!hasActionNameColumn) {
      ...
      await queryRunner.query(
        `ALTER TABLE "permissions" ADD "action_name" varchar`,
      );

      // Update action_name to match action for existing records
      await queryRunner.query(
        `UPDATE "permissions" SET "action_name" = "action"`,
      );

      // Make action_name NOT NULL
      await queryRunner.query(
        `ALTER TABLE "permissions" ALTER COLUMN "action_name" SET NOT NULL`,
      );

      // Rename action column to actionName (for backward compatibility)
      await queryRunner.query(
        `ALTER TABLE "permissions" RENAME COLUMN "action" TO "actionName"`,
      );
    }
    ...
    if (!hasResourceNameColumn) {
      ...
      const hasResourceNameCamelCase = await queryRunner.hasColumn(
        'permissions',
        'resourceName',
      );
      if (hasResourceNameCamelCase) {
        await queryRunner.query(
          `ALTER TABLE "permissions" RENAME COLUMN "resourceName" TO "resource_name"`,
        );
      } else {
        // Add resource_name column if it doesn't exist
        await queryRunner.query(
          `ALTER TABLE "permissions" ADD "resource_name" varchar`,
        );
        await queryRunner.query(
          `UPDATE "permissions" SET "resource_name" = 'unknown' WHERE "resource_name" IS NULL`,
        );
        await queryRunner.query(
          `ALTER TABLE "permissions" ALTER COLUMN "resource_name" SET NOT NULL`,
        );
      }
    }
    ...
    if (!hasRolePermissionsTable) {
      ...
      const hasGrantedColumn = await queryRunner.hasColumn(
        'role_permissions',
        'granted',
      );
      if (!hasGrantedColumn) {
        await queryRunner.query(
          `ALTER TABLE "role_permissions" ADD "granted" boolean DEFAULT true NOT NULL`,
        );
      }
    }

    // Check if group_permissions table exists
    const hasGroupPermissionsTable =
    ...
    if (!hasGroupPermissionsTable) {
      ...
      if (!hasGrantedColumn) {
        await queryRunner.query(
          `ALTER TABLE "group_permissions" ADD "granted" boolean DEFAULT true NOT NULL`,
        );
      }
    }

    // Check if user_permission table exists
    const hasUserPermissionTable =
      await queryRunner.hasTable('user_permission');
    if (!hasUserPermissionTable) {
      ...
      if (!hasGrantedColumn) {
        await queryRunner.query(
          `ALTER TABLE "user_permission" ADD "granted" boolean DEFAULT true NOT NULL`,
        );
      }
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (hasBackupTable) {
      ...
      if (hasActionNameColumn) {
        ...
        const hasActionNameCamelCase = await queryRunner.hasColumn(
          'permissions',
          'actionName',
        );
        if (hasActionNameCamelCase) {
          await queryRunner.query(
            `ALTER TABLE "permissions" RENAME COLUMN "actionName" TO "action"`,
          );
        }

        // Drop the action_name column
        await queryRunner.query(
          `ALTER TABLE "permissions" DROP COLUMN "action_name"`,
        );
      }
      ...
      if (hasResourceNameColumn) {
        ...
        if (!hasResourceNameCamelCase) {
          await queryRunner.query(
            `ALTER TABLE "permissions" RENAME COLUMN "resource_name" TO "resourceName"`,
          );
        }
      }
    }
