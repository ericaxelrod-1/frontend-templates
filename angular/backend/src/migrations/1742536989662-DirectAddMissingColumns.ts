...
export class DirectAddMissingColumns1742536989662 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!permissionsTableExists) {
            ...
            if (permissionTableExists) {
                ...
                console.log('Permissions table created and data copied from permission table');
            } else {
                console.log('Neither permissions nor permission table exists, creating new permissions table');
            ...
            console.log('Permissions table already exists, checking for missing columns');
            ...
            if (!columnNames.includes('resource_name')) {
                ...
                await queryRunner.query(`ALTER TABLE "permissions" ADD COLUMN "resource_name" VARCHAR(50)`);
            }
            
            // Add action column if missing
            if (!columnNames.includes('action')) {
                console.log('Adding action column');
                await queryRunner.query(`ALTER TABLE "permissions" ADD COLUMN "action" VARCHAR(50)`);
            }
