...
export class FixPermissionsTableColumns1742536989660
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!permissionsTableExists) {
      ...
      if (permissionTableExists) {
        console.log(
          'Found old permission table, copying data to new permissions table',
        );
        await queryRunner.query(`
    ...
    const resourceNameColumnExists = await queryRunner.hasColumn(
      'permissions',
      'resource_name',
    );
    if (!resourceNameColumnExists) {
      ...
      console.log(
        'Added resource_name column and populated it from name values',
      );
    } else {
    ...
    const actionColumnExists = await queryRunner.hasColumn(
      'permissions',
      'action',
    );
    ...
    console.log(
      'Successfully updated permissions table with resource_name and action columns',
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      console.log(
        'Note: The columns themselves were not removed to avoid breaking existing code',
      );
    } catch (error) {
