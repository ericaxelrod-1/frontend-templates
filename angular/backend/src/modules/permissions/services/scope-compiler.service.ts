...
export class ScopeCompilerService {
  compileGroup(
    ...
    for (const condition of group.conditions) {
      ...
      if (
        ['IS NULL', 'IS NOT NULL'].includes(condition.operator.toUpperCase())
      ) {
  ...
  private compileGroupRecursive(
    ...
    for (const childGroup of childGroups) {
      const child = this.compileGroupRecursive(
        childGroup,
        `${paramPrefix}_g${idx++}`,
      );
      if (child.sql) {
